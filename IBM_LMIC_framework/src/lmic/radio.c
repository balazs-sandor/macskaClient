...
static void txlora () {
    ...
    lmic_printf("%lu:%lu: TXMODE, freq=%lu, len=%d, SF=%d, BW=%d, CR=4/%d, IH=%d\n",
           millis(), os_getTime(), LMIC.freq, LMIC.dataLen, sf,
...
static void rxlora (u1_t rxmode) {
    ...
    if (rxmode == RXMODE_RSSI) {
        ...
        u1_t bw = getBw(LMIC.rps);
        u1_t cr = getCr(LMIC.rps);
        lmic_printf("%lu:%lu: %s, freq=%lu, SF=%d, BW=%d, CR=4/%d, IH=%d\n",
               millis(), os_getTime(),
...
void radio_irq_handler (u1_t dio) {
    ...
    if( (readReg(RegOpMode) & OPMODE_LORA) != 0) { // LORA modem
        ...
        lmic_printf("%lu:%lu: irq: dio: 0x%x flags: 0x%x\n", m_now, now, dio, flags);
#endif
